...
public class ProductResource {
	 ...
	 *         new product, or with status 400 (Bad Request) if the product has
	 *         already an ID
	...
	public ResponseEntity<Product> createProduct(@Valid @RequestBody Product product) throws URISyntaxException {
		...
		// throw new BadRequestAlertException("A new product cannot already have
		// an ID", ENTITY_NAME, "idexists");
		// }
		
		if (product.getDateAdded()== null){
			product = product.dateAdded(LocalDate.now());
		}
	...
	public ResponseEntity<Product> updateProduct(@Valid @RequestBody Product product) throws URISyntaxException {
		...
		Product old = productService.findOne(product.getId());
		old = old.image(product.getImage())
		...
		if (product.getDateModified()== null){
			old = old.dateModified(LocalDate.now());
		}

		Product result = productService.save(old);
